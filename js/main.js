(function () {
    var PEER_API_KEY = 'af7e49eb-ae1a-45dd-8305-03d5b6c07ca4',
        ...
        START_LAT = 21.869962772770457,
        START_LON = 114.10863485441831,

        DEFAULT_HEIGHT = 24963.09589148224,
        FOG = 0,
        MOVE_SPEED = DEFAULT_HEIGHT / 4,
        ...
        lookLatitude = -0.789977034690008,
    ...
    function updatePosition() {
        ...
        if (vrControls.freeze) {
            if (keys.a) { //look left
                lookLongitude -= Math.PI * delta / 2;
            } else if (keys.d) { //look right
                lookLongitude += Math.PI * delta / 2;
            }

            if (keys.w) { //look up
                lookLatitude = Math.min(0.8 * Math.PI / 2, lookLatitude + Math.PI * delta / 2);
            } else if (keys.s) { //look down
                lookLatitude = Math.max(-0.8 * Math.PI / 2, lookLatitude - Math.PI * delta / 2);
            }
        ...
        if (moving) {
            ...
            if (keys.lift) {
                DEFAULT_HEIGHT < 100000 ? DEFAULT_HEIGHT += DEFAULT_HEIGHT / 1000 : DEFAULT_HEIGHT;
                updateHeight(DEFAULT_HEIGHT);
                dataVizes[''].height = DEFAULT_HEIGHT;
            } else if (keys.down) {
                DEFAULT_HEIGHT > 50 ? DEFAULT_HEIGHT -= DEFAULT_HEIGHT / 1000 : DEFAULT_HEIGHT;
                updateHeight(DEFAULT_HEIGHT);
                dataVizes[''].height = DEFAULT_HEIGHT;
            }
            if (keys.forward) {
            ...
            if (distance) {
                ...
                scratchVector.divideScalar(CITY_SCALE);
    ...
    function initDataViz() {
            ...
            if (!layer) {
                ...
                if (name.indexOf(",") > -1)
                    ...
                    d3.json('layers/' + name + '.json', function (error, data) {
                        ...
                        var temp = ["co", "fsp", "no", "no2", "o3", "rsp", "so2"];
                        if (temp.indexOf(name) > -1) {
                            data.input.options.path = "http://projgw.cse.cuhk.edu.hk:2849/api/" + name.toUpperCase() + "/" + document.getElementById('date').value + "%20" + document.getElementById('time').value + ":00.csv";
                        }

                        if (isZoom) {
                            data.output.options.radius = 5;
                            if (name == "co") {
                                data.output.options.scaleHeight = 0.15;
                            } else {
                                data.output.options.scaleHeight = 1.5;
                            }
                        }
        ...
        activateDataViz = function (name) {
            ...
            */
            if (dataViz === undefined) {
                ...
                window.dataViz(name, {
                    layers: myLayer,
                    info: name.toUpperCase() + ' in Hong Kong'
                });
        ...
        var visualizationAir = document.getElementsByClassName("visualization-air");
        ...
        enterbutton.addEventListener('click', function () {
            ...
            for (var key in layers) {
                if (key != "buildings" && key != "map") {
                    deactivateLayer(key);
                    delete layers[key];
                }
            }
            deactivateLayer(myLayerItems.toString());
            activateDataViz(myLayerItems.toString());
            resize();
            updateHeight(DEFAULT_HEIGHT);
            //updateQuery('viz', myLayerItems.toString());
        });
        ...
        zoomButton.addEventListener('click', function () {
            ...
            //                delete layers[temp[i]];
            //            }
            //            for (var i = 0; i < visualizationAir.length; i++) {
            //                visualizationAir[i].checked = false;
            //            }
            //            deactivateLayer(myLayerItems.toString());
            //            myLayerItems = [];
            if (!isZoom) {
                ...
                camera.far = FAR;
                resize();
                goToLocation(22.2738579 + ', ' + 114.1673188);
                isZoom = true;
            } else {
                DEFAULT_HEIGHT = 24963.09589148224;
                NEAR = 20000;
                ...
                camera.near = NEAR;
                camera.far = FAR;
                searchLocation(21.869962772770457 + ', ' + 114.10863485441831);
                resize();
                isZoom = false;
            }
            activateDataViz("");
        ...
        for (var i = 0; i < visualizationAir.length; i++) {
            visualizationAir[i].addEventListener("click", function () {
                if (this.checked) {
                    if (myLayerItems.indexOf(this.value) == -1) {
                        myLayerItems.push(this.value);
                    }
                    myLayer[this.value] = true;
                    activateDataViz(myLayerItems.toString());
                    //updateQuery('viz', myLayerItems.toString());

                } else {
                    myLayer[this.value] = false;
                    deactivateLayer(myLayerItems.toString());
                    //updateQuery('viz', myLayerItems.toString());
                    var tmp = myLayerItems.indexOf(this.value);
                    if (tmp > -1) {
                        myLayerItems.splice(tmp, 1);
                    }
                    activateDataViz(myLayerItems.toString());
                    //updateQuery('viz', myLayerItems.toString());
    ...
    // fyp
    function goToLocation(val, callback) {
        ...
        function changeLocation(loc) {
            ...
            //reset camera view
            lookLongitude = -Math.PI / 2;
            lookLatitude = -0.38102021100902217;
            lookTarget.y = Math.sin(lookLatitude);
            cos = Math.cos(lookLatitude);
            lookTarget.x = cos * Math.cos(lookLongitude);
            lookTarget.z = cos * Math.sin(lookLongitude);
    ...
    function parseQuery() {
        ...
        if (queryHash.loc) {
            ...
            searchLocation(START_LAT + ', ' + START_LON);
        }
    ...
    function init() {
        ...
        searchbutton.addEventListener('click', function () {
            ...
            camera.far = FAR;
            resize();
            goToLocation(locationInput.value);
            isZoom = true;
        });
